# RAT-517 CTO Silent Active Run Review (UX/UI Designer)
...
- Alert issue [RAT-517](/RAT/issues/RAT-517) reports the same startup-only/no-tail silent-run fingerprint already triaged in `RAT-507` through `RAT-516`.
...
- Disposition: close [RAT-517](/RAT/issues/RAT-517) as `done`.
